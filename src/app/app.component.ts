...
export class AppComponent implements OnInit {
  ...
  menuToggle() {
    ...
    return false;   // Retorna o método que mostra o menu
  }
  ...
  ngOnInit(): void {
    ...
    $(document).ready(() => { // JQuery
      ...
      $(window).resize(() => {
        if (window.innerWidth > 511) {  // Se a largura é maior que 511px
          $('#links').show(0);  //Sempre mostra o menu
        } else {  // Se a largura é menor ou igual a 511px
      ...
      $(document).on('click', '#links > a', () => {
        if (window.innerWidth < 512) {  // Se a largura é menor que 512px
